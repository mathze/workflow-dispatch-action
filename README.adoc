...
you can not use the GITHUB_TOKEN as explained https://docs.github.com/en/actions/writing-workflows/choosing-when-your-workflow-runs/triggering-a-workflow[here]
|*R*
|-
...
|`lookup-tolerance`
|Tolerance to account for server clock skew between the dispatch response time and the workflow run's created_at timestamp.
[NOTE]
The server executing this action may -- and most properly will -- be different from the one that executes the triggered workflow.
[WARNING]
Use with caution as it may result in false positives and may increase requests sent to the api, which in turn could affect your quotas.
|*O*
|0 seconds
